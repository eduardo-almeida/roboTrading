...
class BinanceTraderBot:
    ...
    open_orders = []
    partial_quantity_discount = (
        0  # Valor que já foi executado e que será descontado da quantidade, caso uma ordem não seja completamente executada
    )
    tick_size: float
    step_size: float
    take_profit_index = 0
    ...
    def getStockData(
        ...
        prices["RSI"] = Indicators.getRSI(prices["close_price"], last_only=False)

        # Calcula o Indicador Vortex (VI+ e VI-)
        prices["VI+"] = Indicators.getVortex(prices, window=14, positive=True)
        prices["VI-"] = Indicators.getVortex(prices, window=14, positive=False)
    ...
    def getFinalDecisionStrategy(self):
        final_decision = runStrategies(self, self.stock_data)
